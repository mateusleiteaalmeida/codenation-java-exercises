...
public class Motorista {
    ...
    public static class MotoristaBuilder {
        ...
        public MotoristaBuilder withIdade(int idade) {
            if (idade < 0) throw new IllegalArgumentException("A idade informada não pode ser negativa.");
            this.idade = idade;
            return this;
        }

        public MotoristaBuilder withPontos(int pontos) {
            if (pontos < 0) throw new IllegalArgumentException("Os pontos da habilitação não podem ser negativos.");
            this.pontos = pontos;
            return this;
        }
        ...
        public Motorista build() {
            if (nome == null || nome.isEmpty()) throw new NullPointerException("O nome do motorista deve ser informado.");
            if (habilitacao == null || habilitacao.isEmpty()) throw new NullPointerException("O motorista não possui habilitação.");
            if (idade <= 18) throw new EstacionamentoException("O motorista não possui idade para dirigir.");
            if (pontos > 20) throw new EstacionamentoException("A habilitação do motorista está suspensa.");
